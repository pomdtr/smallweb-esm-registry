  ...
  app.get("/:app", async (c) => {
    ...
    if (config.exports) {
      if (typeof config.exports === "string") {
        return c.redirect(
          path.join("/", `${app}@${oid.slice(0, 7)}`, config.exports),
        );
      }
    ...
    for (const mainFile of entrypoints) {
      try {
        ...
        return c.redirect(
          path.join("/", `${app}@${oid.slice(0, 7)}`, mainFile),
        );
      } catch (_e) {
        continue;
      }
    ...
    throw new HTTPException(404, {
      message: `No default export found for ${app} at ${ref}`,
    });
  });
  ...
  app.get("/:app/:filepath{.+}", async (c) => {
    ...
    if (config.exports && typeof config.exports === "object") {
      ...
      if (key in config.exports) {
        return c.redirect(
          path.join("/", `${app}@${oid.slice(0, 7)}`, config.exports[key]),
        );
      }
    }
    ...
    if (opts.transform && [".js", ".ts", ".jsx", ".tsx"].includes(extname)) {
...
async function findEntryInTree(
  { fs, dir, oid, filepathParts }: {
    fs: git.FsClient;
    dir: string;
    oid: string;
